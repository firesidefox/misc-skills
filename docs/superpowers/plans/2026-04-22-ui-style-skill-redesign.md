# ui-style Skill Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 重写 `misc/skills/ui-style/SKILL.md`，将核心规则前置、压缩 JSX 示例为样式对象、新增 Table 和 Button 两个场景。

**Architecture:** 单文件重写，不拆分。按新章节顺序（核心原则 → 速查卡 → 页面结构 → 布局 Pattern → 卡片 → 表单 → 徽章 → Table → Button → 对比度）重新组织内容，布局 Pattern 改为参数表格形式，卡片改为纯样式对象。

**Tech Stack:** Markdown，React CSSProperties 代码片段
...
---
name: misc-ui-style
description: React Web 页面布局与样式规范。涵盖页面结构、视图布局 Pattern、主题 token 用法、卡片/面板规范、间距原则、状态色方案、数据表格、按钮系统。触发词：布局、样式、UI、卡片、颜色、主题、对比度、呼吸感、视图、页面、表格、按钮。
---
...
- [ ] **Step 3: 验证核心原则在文件前 20 行**

```bash
head -20 misc/skills/ui-style/SKILL.md
```

预期：前 20 行内出现"禁止硬编码颜色值"。

- [ ] **Step 4: 提交**

```bash
git add misc/skills/ui-style/SKILL.md
git commit -m "feat(ui-style): restructure skill - rules first, compress JSX, add table and button"
```

---

### Task 2: 更新 misc/SKILL.md description

**Files:**
- Modify: `misc/SKILL.md`

- [ ] **Step 1: 更新 description 字段，加入 table 和 button**

将 `misc/SKILL.md` 中的 description 行替换为：

```markdown
description: Miscellaneous engineering skills. Sub-skills: shell-logging (colored shell script output), makefile-logging (colored Makefile output), ui-style (web UI layout, style guidelines, table, button system).
```

- [ ] **Step 2: 提交**

```bash
git add misc/SKILL.md
git commit -m "chore(misc): update description to mention table and button"
```
